...
# RUN: ld.lld --icf=all %t1.o -o /dev/null --print-icf-sections | FileCheck %s --implicit-check-not=Z1a --implicit-check-not=Z1b --implicit-check-not=Z1g --implicit-check-not=Z1h
## Test RELA.
# RUN: llvm-mc -filetype=obj -triple=x86_64 %s -o %t2.o
# RUN: ld.lld --icf=all %t2.o -o /dev/null --print-icf-sections | FileCheck %s --implicit-check-not=Z1a --implicit-check-not=Z1b --implicit-check-not=Z1g --implicit-check-not=Z1h
