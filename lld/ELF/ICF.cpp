...
private:
  ...
  struct LsdaInfo {
    ...
    // The personality function, resolved through a DW_EH_PE_indirect thunk if
    // needed; nullptr when the CIE has none. personalityKnown is false when
    // the personality could not be resolved (conservatively not foldable).
...
// Return the symbol referenced by the relocation at `off` in `sec`, or
// nullptr if there is no such relocation (or more than one).
template <class ELFT>
...
// Compare the LSDA/CIE metadata of two sections in terms of their equivalence
// classes.
template <class ELFT>
...
template <class ELFT> void ICF<ELFT>::run() {
  ...
  ctx.in.ehFrame->iterateFDEWithLSDATarget<ELFT>(
      [&](InputSection &text, const CieInfo &cie, const Symbol &sym,
        ...
        LsdaInfo info{lsda, cie.piece->data(), personalityKnown, personality,
                      otherCieRels};
