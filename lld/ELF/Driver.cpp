...
static void findKeepUniqueSections(Ctx &ctx, opt::InputArgList &args) {
  ...
  // A .gcc_except_table section referenced by a live FDE (an LSDA) describes
  // the same behavior as an identical LSDA section, and ICF folds LSDA-bearing
  // functions only when the LSDA and the CIE (personality, encodings, CFI) are
  // equivalent (ICF::lsdaMap). Clear the conservative fallback marking so
  // such sections can be folded; explicit --keep-unique requests and sections
  // listed by the dynsym or a real address-significance table stay unique.
  // The scan does not report .eh_frame errors; finalizeContents does.
